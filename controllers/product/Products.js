...
const newProduct = async (req, res) => {
  const { category, brand, name, price, quantity, desc } = req.body;
  ...
  try {
    ...
    if (!categoryExists) throw "Esta categoria não existe!";
...
const getProductsWithDesc = async(req, res)=>{

  try{
    console.log('teste')
    const products = await Product.where("desc").gt("0")
